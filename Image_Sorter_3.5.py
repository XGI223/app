...
def display_video(video_path, index):
    ...
    for folder in destination_folders:
        ...
        button = tk.Button(
            button_frame,
            text=folder_name,
            command=lambda f=folder, m=video_path, i=index, bi=button_index, v=video: move_media(m, f, i, bi, v),
            width=15,
            height=2,
        )
        button.grid(row=button_index // 2, column=button_index % 2, padx=5, pady=5, sticky="nsew")
    ...
    def update_frame():
        ...
        if not paused:
            ...
            elapsed_time = current_time - update_frame.previous_time if hasattr(update_frame, "previous_time") else 0

            # Calculate the delay until the next frame based on the elapsed time
            delay = int(max(0, (33 - elapsed_time) * 1000))  # Convert to milliseconds

            update_frame.previous_time = current_time  # Store the current time for the next frame

            window.after(delay, update_frame)  # Schedule the next frame update with the adjusted delay
        else:
            update_frame.previous_time = time.time()  # Store the current time for when the video is unpaused
            window.after(33, update_frame)  # Schedule the next frame update

    window.after(33, update_frame)  # Schedule the initial frame update
...
def move_media(media_path, destination_folder, button_index, video):
